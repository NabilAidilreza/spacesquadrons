using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class BarrageTurret : Turret
{
    public bool IsPlayer;
    public GameObject Projectile;
    public float EngageRange;
    public Transform ShootPoint1;
    public Transform ShootPoint2;
    public Transform ShootPoint3;
    public Transform ShootPoint4;
    public Transform ShootPoint5;
    public Transform ShootPoint6;
    public Transform ShootPoint7;
    public Transform ShootPoint8;
    public float CoolDownDuration;
    private float CoolDown;
    private Transform target;
    public float offset;

    public float ShootWindowPeriod;
    private float SWP;
    public float OverHeatDuration;
    public GameObject OH_Effect;
    private GameObject OH;
    public GameObject ShockEffect;
    private GameObject Shock;
    // Start is called before the first frame update
    void Start()
    {
        CoolDown = CoolDownDuration;
        SWP = ShootWindowPeriod;
        status = "Online";
    }

    // Update is called once per frame
    void Update()
    {
        if (status == "Online")
        {
            MainFunc();
        }
        else if (status == "Offline")
        {
            DisableTurret();
            status = "Null";
        }
    }
    IEnumerator Overheat()
    {
        yield return new WaitForSeconds(OverHeatDuration);
        SWP = ShootWindowPeriod;
    }
    IEnumerator Recover()
    {
        yield return new WaitForSeconds(5f);
        status = "Online";
    }
    public void DisableTurret()
    {
        Shock = Instantiate(ShockEffect, new Vector3(transform.position.x, transform.position.y, transform.position.z - 10), Quaternion.identity);
        StartCoroutine("Recover");
    }
    IEnumerator ShootAgain(Transform target)
    {
        if(target != null)
        {
            Barrage projectile = Instantiate(Projectile, ShootPoint1.position, Quaternion.identity).GetComponent<Barrage>();
            projectile.SetTarget(target);
            Barrage projectile2 = Instantiate(Projectile, ShootPoint2.position, Quaternion.identity).GetComponent<Barrage>();
            projectile2.SetTarget(target);
            yield return new WaitForSeconds(0.2f);
            Barrage projectile3 = Instantiate(Projectile, ShootPoint3.position, Quaternion.identity).GetComponent<Barrage>();
            projectile3.SetTarget(target);
            Barrage projectile4 = Instantiate(Projectile, ShootPoint4.position, Quaternion.identity).GetComponent<Barrage>();
            projectile4.SetTarget(target);
            yield return new WaitForSeconds(0.2f);
            Barrage projectile5 = Instantiate(Projectile, ShootPoint6.position, Quaternion.identity).GetComponent<Barrage>();
            projectile5.SetTarget(target);        
            Barrage projectile6 = Instantiate(Projectile, ShootPoint6.position, Quaternion.identity).GetComponent<Barrage>();
            projectile6.SetTarget(target);
            yield return new WaitForSeconds(0.2f);
            Barrage projectile7 = Instantiate(Projectile, ShootPoint7.position, Quaternion.identity).GetComponent<Barrage>();
            projectile7.SetTarget(target);
            Barrage projectile8 = Instantiate(Projectile, ShootPoint8.position, Quaternion.identity).GetComponent<Barrage>();
            projectile8.SetTarget(target);
        }

    }
    public void MainFunc()
    {
        target = FindClosestObject();
        if (target == null)
        {
            target = FindClosestObject();
        }
        else
        {
            float Range = Vector2.Distance(transform.position, target.transform.position);
            if (Range < EngageRange)
            {
                if (SWP < 0f)
                {
                    OH = Instantiate(OH_Effect, new Vector3(transform.position.x, transform.position.y, transform.position.z - 10), Quaternion.identity);
                    StartCoroutine("Overheat");
                    SWP = 0f;
                }
                else if (SWP > 0f)
                {
                    if (CoolDown <= 0)
                    {
                        StartCoroutine(ShootAgain(target));
                        CoolDown = CoolDownDuration;
                    }
                    else
                    {

                        CoolDown -= Time.deltaTime;
                    }
                    SWP -= Time.deltaTime;
                }
            }
        }
    }
    public Transform FindClosestObject()
    {
        if (IsPlayer == true)
        {
            float DtoC = Mathf.Infinity;
            Enemy closestEnemy = null;
            Enemy[] lall = GameObject.FindObjectsOfType<Enemy>();
            foreach (Enemy curr in lall)
            {
                float DtoE = (curr.transform.position - this.transform.position).sqrMagnitude;
                if (DtoE < DtoC)
                {
                    DtoC = DtoE;
                    closestEnemy = curr;
                }
            }
            if (closestEnemy == null)
            {
                return null;
            }
            return closestEnemy.GetComponent<Transform>();
        }
        else
        {
            float DtoC = Mathf.Infinity;
            Player closestPlayer = null;
            Player[] lall = GameObject.FindObjectsOfType<Player>();
            foreach (Player curr in lall)
            {
                float DtoE = (curr.transform.position - this.transform.position).sqrMagnitude;
                if (DtoE < DtoC)
                {
                    DtoC = DtoE;
                    closestPlayer = curr;
                }
            }
            if (closestPlayer == null)
            {
                return null;
            }
            return closestPlayer.GetComponent<Transform>();
        }
    }
}
